...
int deciderScore(int *game, int meAI)
{
    ...
    if(game[2] == game[4] && game[4] == game[8])
    {
        if(meAI == game[2])
        {
            return 10;
...
int minmaxAlgorithm(int *game, int meAI, bool isX)
{
    ...
    for(int i=0;i<9;i++){
        if(game[i] < 1){
            if(isX){
                ...
                game[i] = 1;
                cout<< "bb X" << endl;
                int value = minmaxAlgorithm(game, meAI, !isX) ;
                cout << value << endl;
                matrixOutput(game);
                game[i] = temp;
                return value;
...
int aiTurnPro(int *game, int meAI)
{
    ...
    for (int i=0; i<9; i++)
    {
        if(game[i] < 1)
        {
            ...
            int best = minmaxAlgorithm(game, meAI, aicc);
